...
def compute_rewrite_quality_counterfact_generation(
    ...
    gens = generate_fast(model, tok, generation_prompts, max_out_len=50)
    truths = [record["requested_rewrite"]["prompt"].format(subject) + ' %s' %target_true["str"]] * len(gens)
    counterfactuals = [record["requested_rewrite"]["prompt"].format(subject) + ' %s' %target_new["str"]] * len(gens)
